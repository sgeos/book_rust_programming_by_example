...
impl<T: BitAnd<Output=T> + BitAndAssign + BitOrAssign + Copy + FromPrimitive + Not + PartialEq + Shl<T> + Shr<usize, Output=T>> BitSet for T {
    fn clear(&mut self, index: usize) {
        *self &= T::from_usize(!(1 << index)).unwrap();
    }

    fn is_set(&self, index: usize) -> bool {
        (*self >> index) & T::from_usize(1).unwrap() == T::from_usize(1).unwrap()
    }
